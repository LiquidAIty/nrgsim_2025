...
void contractPrint(CONTRACT* c,int To)
{
	...
	print(To,"\n%s,%s%s%s,%i,%s,%s,%s,%s,%s,%s,%s",
		...
		sftoa(c->fVal,3),
		sftoa(c->fVol,3),
		ifelse(is(TRADEMODE),(const char*)strcon(c),""));
}
...
var contractStrike(int Type,var Days,var Price,var HistVol,var RiskFree,var Delta)
{
...
var contractDelta(int Type,var Days,var Price,var HistVol,var RiskFree,var Strike)
{
...
var contractVega(int Type,var Days,var Price,var HistVol,var RiskFree,var Strike)
{
...
TRADE* contractRoll(TRADE* tr,int Days,var Strike,intptr_t TMF)
{
...
void plotContract(int N,CONTRACT* C,var HistVol,var Min,var Max,int Days,int Modus)
{
	...
	for(i=0; i<CPOINTS; i++)
	{
		if(N == 0 || C == 0 || Modus == 0) {
			Values[i] = 0;
			plotBar("#Zero",i,NIL,0,LINE,BLACK);
			continue;
		} 
...
// Date (0),Open Interest (f1),Producer/Merchant/Processor/User Longs(f2),Producer/Merchant/Processor/User Shorts(f3),
// Swap Dealer Longs(f4),Swap Dealer Shorts(f5),Swap Dealer Spreads,Money Manager Longs(f6),Money Manager Shorts(f7),Money Manager Spreads,
