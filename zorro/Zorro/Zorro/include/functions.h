...
C int F0(assetAdd)(const char* Name,var vPrice,var vSpread,var vRollLong,var vRollShort,var vPip,var vPipCost,var vMarginCost,var vLeverage,var vLotAmount,var vCommission,const char* sSymbol);
...
C var F0(brokerCommand)(int Cmd,intptr_t Parameter);
// panel
...
C size_t F(file_read)(const char* name,char* content,size_t size);
C size_t F0(file_append)(const char* name,const char* content);
C size_t F1(file_append)(const char* name,const void* content,size_t size);
C size_t F(file_write)(const char* name,const char* content,size_t size);
C long F(file_copy)(const char* dest,const char* src);
C int F(file_delete)(const char* name);
C size_t F(file_length)(const char* name);
...
C int F(http_status)(int id);
C size_t F(http_result)(int id,char* content, size_t size);
...
C int F(dataSaveCSV)(int Handle,const char* Format,const char* Name,int Start,int Num,...);
...
C var F(ccyStrength)(const char* Currency);
...
C int F(stridx)(const char* Name,int Index);
...
C int F(dataCompress)(int Handle,const char* FileName,int Fields,var Resolution);
...
C int F(priceRecord)();
C size_t F(file_appendfront)(const char* name,const void* content, size_t size);
...
C void F(debuginfo)(const char* s);
...
C const char* F(report)(int Mode);
C var F(RET)(int TimePeriod);
C void F(pyExit)(int Enforce);
...
C char* F1(strx)(char *Out,size_t Size,const char* In,const char* Orig,const char* Repl);
C int F(zInit)(int Num,int Size);
C void* F(zData)(int Id);
C HANDLE F(zOpen)(int Id, const char* Commandline);
C void F(zClose)(int Id);
C int F(zStatus)(int Id);
C var F0(erf)(var x);

C int F(once)(BOOL Condition);
C void* F(zalloc)(size_t Size);
...
C void F(dataChart)(int Handle,const char* name,int Type,const char* Labels);
C char* F0(of)(void* p1,...);

// 2.44
C int F(tradeUpdate)(TRADE* Trade,long NewLots,var NewStop,var NewTakeProfit,long NewLifeTime);
C int F(MatchIndex)(vars Data,var Val,int Period); 

// 2.48
C int F(at)(int HHMM);
C int F(barssince)(BOOL Condition);
C var F(valuewhen)(BOOL Condition,vars Data,int Occurrence);
C int F(dataDelete)(int Handle,int Row1,int Row2);
C var F1(ADX)(var *Open,var *High,var *Low,var *Close,int TimePeriod);
C var F1(DX)(var *Open,var *High,var *Low,var *Close,int TimePeriod);
C var F(SumSq)(var* Data,int Length);
C int F(dataRow)(int Handle,void* Record);
C char* F2(strdate)(const char* Format,int Zone,int Offset);

// 2.50
C int F(watch)(const char* Text,...);
C var F1(brokerCommand)(int Cmd,void* Ptr);
C int F(dataCopy)(int Handle,var* Data);
C char* F(pyStr)(const char* Name);
C int F3(pySet)(const char* Name,const char* s);
C int F(plotHistogram)(const char* Name,var Val,var Step,var Weight,int Color);
C void F1(cancelTrade)(TRADE*);

// 2.53
C var F(cum)(var Inc);
C var F(fix0)(var Val);
C var F(priceReal)();
C int F(changed)(var Val);
C char* F(hmac)(const char* Msg, int Length, const char* Secret, int Mode);
C int F(http_request)(const char* Url,const char* Data,const char* Header, const char* Method);
C char* F(brokerRequest)(const char* Path,const char* Method,const char* Data);
C var F(SMMA)(vars Data,int TimePeriod);

//2.56
C char* F(strcatf)(char* str,const char* insert);
C var F(sync)(vars Data,int Type,int Period);

//2.64
C int F(NumAbove)(var* Data,int Period,var vThreshold);
C var F(VolatilityP)(int Period);
