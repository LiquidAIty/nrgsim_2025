...
int plot2(string Name,var Value,int Type,int Select,int Color1,int Color2)
{
	plot(strf("#%s",Name),Value,Type,Color2);
	if(is(FIRSTRUN)) // put it in the right subchart
		plot(Name,NIL,Type,Color1);
	if(Select)
		plot(Name,Value,Type,Color1);
	return Select;
}
