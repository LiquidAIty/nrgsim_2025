...
{ 
	if(mode == 0)
		...
		print(TO_ANY,"-> R error - %s",RLastOutput(hR));
		quit("!");
		hR = 0;
		return 0;
	} 	
...
// export batch training samples to a file to be read by R	
	if(mode == NEURAL_TRAIN) {
		print(TO_ANY,"\nNEURAL_TRAIN %i",model+1);
		Rset("AlgoVar",AlgoVar,8);
		string name = strf("Data\\signals%i.csv",Core);
		file_write(name,(char*)Data,0);
		if (!Rx(strf("XY <- read.csv('%s%s',header = F)",slash(ZorroFolder),slash(name)),0)) {
			print(TO_ANY,"\nCan't read %s - ",name); return 0;
		}
		if (!Rx(strf("neural.train(%i,XY)",model+1),2)) {
			print(TO_ANY,"\nTrain %s - ",name); return 0;
		}
		Rset("AlgoVar1",AlgoVar,8);
		if (!Rx(strf("neural.train(%i,XY)",model+2),2)) {
			print(TO_ANY,"\nTrain again - "); return 0;
		}
		Rset("AlgoVar2",AlgoVar,8);
	}
// predict the target	
	if(mode == NEURAL_PREDICT) {
		print(TO_ANY,"\nNEURAL_PREDICT %i",model+1);
		Rset("AlgoVar",AlgoVar,8);
...
// save all trained models	
	if(mode == NEURAL_SAVE) {
		...
		if (!Rx(strf("neural.save('%s')",slash((char*)Data)),2)) {
			print(TO_ANY," - failed"); return 0;
		}
	}
// load all trained models	
	if(mode == NEURAL_LOAD) {
		...
		if(Exists) Exists = Rx(strf("neural.load('%s')",slash((char*)Data)),2);
 		else Exists = Rx(strf("load('%s')",slash((char*)Data)),2);
		if (!Exists) {
			print(TO_ANY," - failed"); return 0;
		}
 	}
